<template>
    <nuc-page-container header-label="Interaction 交互" no-footer>
        <nuc-chart :source="data1" :scale="scale1">
            <nuc-legend position="right" marker="square" />
            <nuc-coord type="polar" :transposed="true" :radius="0.85" :inner-radius="0.618" />
            <nuc-axis :visible="false" />
            <nuc-geom type="interval" position="a*percent" color="name" adjust="stack" />
            <nuc-guide>
                <nuc-guide-html :position="['50%', '50%']" :html="pieGuideHtml" />
            </nuc-guide>
            <nuc-interaction>
                <nuc-interaction-pie-select
                    :animate="{ duration: 300, easing: 'backOut' }"
                    :default-selected="{ name: '机器之血', percent: 0.18, a: '1' }"
                    @on-end="pieSelectOnEnd"
                />
            </nuc-interaction>
        </nuc-chart>
        <nuc-chart :source="data2" :scale="{ sales: { tickCount: 5 } }">
            <nuc-geom type="interval" position="year*sales" />
            <nuc-interaction>
                <nuc-interaction-interval-select
                    :select-axis-style="{ fill: '#000', fontWeight: 'bold' }"
                    mode="range"
                    :default-selected="{ year: '1962 年', sales: 38 }"
                />
            </nuc-interaction>
        </nuc-chart>
        <nuc-chart :source="data3" :scale="{ release: { min: 1990, max: 2010 } }">
            <nuc-tooltip
...
export default {
    ...
    computed: {
        pieGuideHtml () {
            return '<div style="text-align: center;width: 100px;height: 72px;vertical-align: middle;">' + '<p id="number" style="font-size: 28px;margin: 10px 10px 5px;font-weight: bold;"></p>' + '<p id="name" style="font-size: 12px;margin: 0;"></p>' + '</div>'
        }
    },
    methods: {
        pieSelectOnEnd (ev) {
            ...
            if (shape) {
                if (selected) {
                    window.$('#number').css('color', shapeInfo.color)
                    window.$('#number').text(data.percent * 100 + '%')
                    window.$('#name').text(data.name)
                } else {
                    window.$('#number').text('')
                    window.$('#name').text('')
                }
